...
## 📐 1. Dual-Density Self-Sharpening Blade Metrics
To ensure the cutting teeth natively sharpen themselves using wood friction without manual filing or replacement loops, the 3D-printed matrix requires these strict boundaries:
*   **Primary Tooth Pitch Spacing Profile:** Exactly 8.5 mm segment-to-segment.
*   **Leading-Edge Material Layer:** Zinc-infused photopolymer tough resin ($\geq 3.2\text{ GPa}$ flexural modulus // Hardness $\geq 88\text{ Shore D}$).
*   **Trailing-Face Material Layer:** Flexible un-reinforced elastomeric resin ($\approx 1.2\text{ GPa}$ flexural modulus // Hardness $\approx 60\text{ Shore D}$).
*   **Differential Wear-Rate Calibration Window:** Outlined at a strict $2.6:1$ ratio, forcing the trailing face to drop away faster under friction, which continuously maintains a sub-micron ($\leq 0.5\text{ \mu m}$) cutting vertex edge software-free.
