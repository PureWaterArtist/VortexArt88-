...
def run_spacetime_simulation(
    ...
    for current_ma in solenoid_excitation_current_amps_ma:
        ...
        # As magnetic plasma currents scale, the local spacetime curvature begins to warp non-linearly
        if current_ma == 0:
            metric_distortion_factor = 0.0
            throat_radius_m = 0.0
        else:
            # Metric shifts scale quadratically driven by multi-Tesla field energy-momentum densities
            ...
            # Extrapolate theoretical geometric Einstein-Rosen throat aperture scale
        ...
        # High fluid kinetic vorticity twists native temporal axis coordinates into safe closed intervals
        if current_ma == 0:
            temporal_displacement_seconds = 0.0
            causal_status_str = "Linear Flow"
        else:
            # Temporal shifts track rotational field momentum, bounded by causal dampening constants
        ...
        # Quantifies configuration mapping coherence across paired superfluid receiver matrices
        if current_ma == 0:
            teleportation_fidelity_pct = 0.0
        else:
            # Quantum tracking metrics optimize asymptotically up to an absolute 99.999% reconstruction cap
        ...
        # Format outputs cleanly matching resting baselines vs active running boundaries
...
if __name__ == "__main__":
    # Test sweep from standby (0 MA) up to high-energy core spacetime manipulation currents (50 MA)
    solenoid_excitation_current_amps_ma = np.array([0.0, 5.0, 15.0, 25.0, 38.0, 50.0])
    run_spacetime_model = run_spacetime_simulation(solenoid_excitation_current_amps_ma)
